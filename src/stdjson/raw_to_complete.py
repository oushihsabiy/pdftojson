...
- Rewrite only the "problem" field to make it self-contained and condition-complete.
...
def find_prompt_path(prompt_arg: Optional[str]) -> Path:
    ...
    ).parents[1] / "prompt" / "raw_to_complete.md"
...
def chat_completion_text(
    ...
    # Prefer JSON mode when the backend supports it.
    try:
...
def complete_problem_field(
    ...
    raise RuntimeError(
        f"Failed to obtain a valid rewritten problem after {max_attempts} attempts: {last_error}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Rewrite only the 'problem' field of exercise JSON objects using the raw_to_complete prompt.",
    )
    ...
    parser.add_argument(
        "--prompt",
        default=None,
        help="Path to the prompt markdown file. Defaults to src/prompt/raw_to_complete.md.",
    )
    parser.add_argument(
        "--model",
        default=None,
...
def main() -> None:
    ...
    for index, exercise in enumerate(exercises, start=1):
        ...
        print(f"[{index}/{total}] rewriting problem for {label}", file=sys.stderr)
        try:
            new_problem = complete_problem_field(
                client,
                model=model,
            ...
            # Keep original problem text for this item to avoid aborting the whole job.
            failed_labels.append(str(label))
            print(
                f"[warn] rewriting failed for {label}; keep original problem. reason={err}",
                file=sys.stderr,
            )

    write_json(output_path, data)
    print(f"Wrote {total} completed exercise(s) to {output_path}",
          file=sys.stderr)
    if failed_labels:
        print(
