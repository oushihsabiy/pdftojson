...
Definition / Hypothesis / Goal format suitable for Lean formalization.
Uses the concise_to_lean prompt.

Usage (standalone):
    python concise_to_lean.py input.json output.json [--prompt PATH] [--model NAME]
...
def find_prompt_path(prompt_arg: Optional[str]) -> Path:
    ...
    ).parents[1] / "prompt" / "concise_to_lean.md"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--prompt",
        default=None,
        help="Path to the prompt markdown file. Defaults to src/prompt/concise_to_lean.md.",
    )
    parser.add_argument(
        "--model",
        default=None,
